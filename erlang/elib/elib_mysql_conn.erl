...
init(Host, Port, User, Password, Database, LogFun, Encoding, PoolId, Parent) ->
    case elib_mysql_recv:start_link(Host, Port, LogFun, self()) of
	{ok, RecvPid, Sock} ->
		...
		{ok, Version} ->
			...
			{error, MySQLRes} ->
			    ?Log2(LogFun, error,
				 ...
				 "~p : ~p",
				 [Database,
				  elib_mysql_impl:get_result_reason(MySQLRes)]),
			...
			{_ResultType, _MySQLRes} ->
			    Parent ! {mysql_conn, self(), ok},
			    		%%changgb 强制utf8
  				    do_query(Sock, RecvPid, LogFun,<<"set names 'utf8'">>,Version),
...
do_execute(State, Name, Params, ExpectedVersion) ->
    Res = case gb_trees:lookup(Name, State#state.prepares) of
		  ...
		  {ok, latest};
	      {value, Version} ->
		  elib_mysql_impl:get_prepared(Name, Version);
	      none ->
		  elib_mysql_impl:get_prepared(Name)
	  end,
    case Res of
	{ok, latest} ->
...
make_statements_for_execute(Name, Params) ->
    ...
    Stmts = lists:foldl(
	      fun({Num, Val}, Acc) ->
		      NumBin = elib_mysql_impl:encode(Num, true),
		      ValBin = elib_mysql_impl:encode(Val, true),
...
asciz(Data) when is_binary(Data) ->
    elib_mysql_impl:asciz_binary(Data, []);
