...
connect(PoolId, Host, Port, User, Password, Database, Encoding, Reconnect,
    ...
    Fun = if LinkConnection ->
		  fun elib_mysql_conn:start_link/8;
	     true ->
		  fun elib_mysql_conn:start/8
	  end,
...
fetch(Query) ->
	...
	State ->
	    elib_mysql_conn:fetch_local(State, Query)
    end.
...
execute(Name, Params) when is_atom(Name), is_list(Params) ->
	...
	undefined ->
	    {error, not_in_transaction};
	State ->
	    elib_mysql_conn:execute_local(State, Name, Params)
    end;
...
execute(PoolId, Name, Params, Timeout) ->
	...
	State ->
	      case elib_mysql_conn:execute_local(State, Name, Params) of
		  {ok, Res, NewState} ->
...
transaction(PoolId, Fun, Timeout) ->
	...
	State ->
	    case elib_mysql_conn:get_pool_id(State) of
		PoolId ->
		    case catch Fun() of
			error = Err -> throw(Err);
...
init([PoolId, Host, Port, User, Password, Database, LogFun, Encoding]) ->
    ...
    case elib_mysql_conn:start(Host, Port, User, Password, Database, LogFun1,
			  Encoding, PoolId) of
	{ok, ConnPid} ->
...
handle_call({execute, PoolId, Name, Params}, From, State) ->
    with_next_conn(
      ...
      fun(Conn, State1) ->
		  ...
		  {value, {_Stmt, Version}} ->
		      elib_mysql_conn:execute(Conn#conn.pid, Name,
					 Version, Params, From),
		      {noreply, State1}
	      end
      end);
...
handle_call({transaction, PoolId, Fun}, From, State) ->
    with_next_conn(
      PoolId, State,
      fun(Conn, State1) ->
	      elib_mysql_conn:transaction(Conn#conn.pid, Fun, From),
	      {noreply, State1}
      end);
...
fetch_queries(PoolId, From, State, QueryList) ->
    with_next_conn(
      ...
      fun(Conn, State1) ->
	      Pid = Conn#conn.pid,
	      elib_mysql_conn:fetch(Pid, QueryList, From),
	      {noreply, State1}
      end).
